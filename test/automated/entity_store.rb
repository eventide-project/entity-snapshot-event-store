...
context "Entity Store" do
  ...
  context "Projecting #{count} Events [Written: #{total}, Interval: #{snapshot_interval}, Mod: #{total % snapshot_interval}]" do
    ...
    context "Writes first snapshot" do
      snapshot_message = MessageStore::EventStore::Get.(snapshot_stream_name, position: snapshot_position, batch_size: 1).first

      entity_data = snapshot_message.data[:entity_data]
      sum = entity_data[:sum]
      entity_version = snapshot_message.data[:entity_version]
